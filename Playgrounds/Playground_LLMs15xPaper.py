...
def preprocess_text_with_LLM(doc):
    ...
    Implicit actions are often formed by using the participle. Transform the participle into an explicit action.
        ...
        ### Instruction: ####
        Solve the tasks carefully step by step. Do not comment on any of the steps. Return the sentence without interpretations.
...
def write_to_file(number: int, nlp):
    input_path = f"/Users/vincentderekheld/PycharmProjects/bachelor-thesis/Evaluation/Approach-15xPaper/INPUT_Text/text_{number}.txt"
    text_input = open(input_path, 'r').read().replace("\n", " ")
    doc = nlp(text_input)
    content = preprocess_text_with_LLM(doc)
    output_path = f"/Users/vincentderekheld/PycharmProjects/bachelor-thesis/Evaluation/Approach-15xPaper/LLM_preprocessed_text/Text{number.__str__()}.txt"
    with open(output_path, 'w') as file:
        file.write(content)
